...
def get_schedules(schedule_req: ScheduleRequest) -> Dict[str, Any]:
    ...
    body = _create_schedule_request_body(schedule_req)
    return graph_api_client.get_schedules(target_user_email, body)

def _create_schedule_request_body(schedule_req: ScheduleRequest) -> Dict[str, Any]:
...
def parse_availability(schedule_data: Dict[str, Any], start_hour: float, end_hour: float) -> List[List[Tuple[float, float]]]:
    ...
    return [
        _get_free_slots(schedule, start_hour, end_hour)
        for schedule in schedules_info
    ]

def _get_free_slots(schedule: Dict[str, Any], start_hour: float, end_hour: float) -> List[Tuple[float, float]]:
    """個別のスケジュールから空き時間を抽出"""
    slot_duration = 0.5
...
def create_event_payload(appointment_req: AppointmentRequest, start_str: str, end_str: str) -> Dict[str, Any]:
    """イベントペイロードを生成"""
    return {
        "subject": _create_event_subject(appointment_req),
        "body": {
            "contentType": "HTML",
            "content": _create_event_body(appointment_req),
        },
...
def _create_event_subject(appointment_req: AppointmentRequest) -> str:
    """イベントの件名を生成"""
    return f"【{appointment_req.company}/{appointment_req.lastname}{appointment_req.firstname}様】日程確定"

def _create_event_body(appointment_req: AppointmentRequest) -> str:
    """イベントの本文を生成"""
    return (
