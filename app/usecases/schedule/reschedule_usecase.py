...
async def reschedule_usecase(reschedule_req: RescheduleRequest) -> None:
    """リスケジュール処理を行うユースケース"""
    try:
        cosmos_db_client = AzCosmosDBClient()
        form_data = cosmos_db_client.get_form_data(reschedule_req.cosmos_db_id)
        ...
        if reschedule_req.schedule_interview_datetime is None:
            ...
            appointment_repository.delete_appointment(reschedule_req.cosmos_db_id)
            logger.info(f"DBレコード削除成功: {reschedule_req.cosmos_db_id}")
            
            # CosmosDBのフォームデータを削除
            cosmos_db_client.delete_form_data(reschedule_req.cosmos_db_id)
            logger.info(f"CosmosDBレコード削除成功: {reschedule_req.cosmos_db_id}")
            
            _send_no_available_reschedule_emails(reschedule_req.cosmos_db_id)
            return
        ...
        appointment_repository = AppointmentRepository()
        appointment_repository.update_schedule_interview_datetime(reschedule_req.cosmos_db_id, reschedule_req.schedule_interview_datetime)
        ...
        # リスケジュール完了メールを送信
        _send_reschedule_emails(reschedule_req.cosmos_db_id, reschedule_req.schedule_interview_datetime)
...
def _send_reschedule_emails(cosmos_db_id: str, schedule_interview_datetime: str | None) -> None:
    ...
    # DBからアポイントメントデータを取得
    appointment_repository = AppointmentRepository()
    appointment_data = appointment_repository.get_appointment_by_cosmos_db_id(cosmos_db_id)
...
def _send_no_available_reschedule_emails(cosmos_db_id: str) -> None:
